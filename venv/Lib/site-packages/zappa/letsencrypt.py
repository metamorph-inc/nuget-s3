...
def get_cert_and_update_domain(
    ...
    try:
        ...
        with open('_tmp_signed.crt') as f:
            certificate_body = f.read()

        with open('_tmp_domain.key') as f:
            certificate_private_key = f.read()

        with open('_tmp_intermediate.pem') as f:
...
def create_domain_key():
    ...
    proc = subprocess.Popen(
        "openssl genrsa 2048 > _tmp_domain.key".split(),
...
def create_domain_csr(domain):
    subj = "/CN=" + domain
    cmd = 'openssl req -new -sha256 -key _tmp_domain.key -subj {}  > _tmp_domain.csr'.format(subj)
    proc = subprocess.Popen(
        cmd.split(),
...
def create_chained_certificate():
    ...
    with open('_tmp_intermediate.pem', 'wb') as intermediate_pem:
        intermediate_pem.write(cert.content)

    proc = subprocess.Popen(
        "cat _tmp_signed.crt _tmp_intermediate.pem > _tmp_chained.pem".split(),
        stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    out, err = proc.communicate()
    if proc.returncode != 0: # pragma: no cover
...
def parse_account_key():
    ...
    proc = subprocess.Popen(
        "openssl rsa -in _tmp_account.key -noout -text".split(),
        stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    out, err = proc.communicate()
    if proc.returncode != 0: # pragma: no cover
...
def parse_csr():
    ...
    csr_filename = '_tmp_domain.csr'
    proc = subprocess.Popen(
        "openssl req -in {} -noout -text".format(csr_filename).split(),
...
def sign_certificate():
    ...
    proc = subprocess.Popen(
        "openssl req -in _tmp_domain.csr -outform DER".split(),
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
...
def encode_certificate(result):
    ...
    signed_crt = open("_tmp_signed.crt", "w")
...
def _send_signed_request(url, payload):
    ...
    proc = subprocess.Popen(
        "openssl dgst -sha256 -sign _tmp_account.key".split(),
        stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
